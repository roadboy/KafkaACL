...
class LogOffsetTest extends JUnit3Suite with ZooKeeperTestHarness {
  ...
  def testGetOffsetsForUnknownTopic() {
    ...
    val request = OffsetRequest(
      Map(topicAndPartition -> PartitionOffsetRequestInfo(OffsetRequest.LatestTime, 10)),aclAuth = null, zkConnect=null)
  ...
  def testGetOffsetsBeforeLatestTime() {
    ...
    val offsetRequest = OffsetRequest(
      ...
      replicaId = 0,aclAuth = null, zkConnect=null)
    val consumerOffsets =
  ...
  def testEmptyLogsGetOffsets() {
    ...
    // setup brokers in zookeeper as owners of partitions for this test
    createTopic(zkClient, topic, numPartitions = 1, replicationFactor = 1, servers = Seq(server), acl=null)

    var offsetChanged = false
    for(i <- 1 to 14) {
      val topicAndPartition = TopicAndPartition(topic, 0)
      val offsetRequest =
        OffsetRequest(Map(topicAndPartition -> PartitionOffsetRequestInfo(OffsetRequest.EarliestTime, 1)),aclAuth = null, zkConnect=null)
      val consumerOffsets =
        simpleConsumer.getOffsetsBefore(offsetRequest).partitionErrorAndOffsets(topicAndPartition).offsets
  ...
  def testGetOffsetsBeforeNow() {
    ...
    // setup brokers in zookeeper as owners of partitions for this test
    AdminUtils.createTopic(zkClient, topic, 3, 1, acl=null)
    ...
    waitUntilTrue(() => isLeaderLocalOnBroker(topic, part, server), "Leader should be elected")
    val topicAndPartition = TopicAndPartition(topic, part)
    val offsetRequest = OffsetRequest(Map(topicAndPartition -> PartitionOffsetRequestInfo(now, 10)), replicaId = 0,aclAuth = null, zkConnect=null)
    val consumerOffsets =
      simpleConsumer.getOffsetsBefore(offsetRequest).partitionErrorAndOffsets(topicAndPartition).offsets
  ...
  def testGetOffsetsBeforeEarliestTime() {
    ...
    val topicAndPartition = TopicAndPartition(topic, part)
    val offsetRequest =
      OffsetRequest(Map(topicAndPartition -> PartitionOffsetRequestInfo(OffsetRequest.EarliestTime, 10)),aclAuth = null, zkConnect=null)
    val consumerOffsets =
      simpleConsumer.getOffsetsBefore(offsetRequest).partitionErrorAndOffsets(topicAndPartition).offsets
