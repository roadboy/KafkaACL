...
class DynamicConfigChangeTest extends JUnit3Suite with KafkaServerTestHarness {
  ...
  def testConfigChange() {
    ...
    AdminUtils.createTopic(zkClient, tp.topic, 1, 1, LogConfig(flushInterval = oldVal).toProps, acl=null)
    TestUtils.retry(10000) {
      val logOpt = this.servers(0).logManager.getLog(tp)
      assertTrue(logOpt.isDefined)
      assertEquals(oldVal, logOpt.get.config.flushInterval)
    }
    AdminUtils.changeTopicConfig(zkClient, tp.topic, LogConfig(flushInterval = newVal).toProps, acl=null)
    TestUtils.retry(10000) {
  ...
  def testConfigChangeOnNonExistingTopic() {
    ...
    try {
      AdminUtils.changeTopicConfig(zkClient, topic, LogConfig(flushInterval = 10000).toProps, acl=null)
