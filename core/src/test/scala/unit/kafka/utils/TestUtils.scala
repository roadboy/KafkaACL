...
object TestUtils extends Logging {
  ...
  def createTopic(zkClient: ZkClient,
                  ...
                  acl: java.util.List[ACL]) : scala.collection.immutable.Map[Int, Option[Int]] = {
    // create topic
    AdminUtils.createTopic(zkClient, topic, numPartitions, replicationFactor, topicConfig, acl=acl)
  ...
  def createTopic(zkClient: ZkClient, topic: String, partitionReplicaAssignment: collection.Map[Int, Seq[Int]],
                  ...
                  acl: java.util.List[ACL]) : scala.collection.immutable.Map[Int, Option[Int]] = {
    // create topic
    AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, topic, partitionReplicaAssignment, acl=acl)
    // wait until the update metadata request for new topic reaches all servers
  ...
  def updateConsumerOffset(config : ConsumerConfig, path : String, offset : Long, acl: java.util.List[ACL]) = {
    val zkClient = new ZkClient(config.zkConnect, config.zkSessionTimeoutMs, config.zkConnectionTimeoutMs, ZKStringSerializer)
    ZkUtils.updatePersistentPath(zkClient, path, offset.toString, acl=acl)
  ...
  def createBrokersInZk(zkClient: ZkClient, ids: Seq[Int], acl: java.util.List[ACL]): Seq[Broker] = {
    val brokers = ids.map(id => new Broker(id, "localhost", 6667))
    brokers.foreach(b => ZkUtils.registerBrokerInZk(zkClient, b.id, b.host, b.port, 6000, jmxPort = -1, acl=acl))
    brokers
  }
  ...
  def makeLeaderForPartition(zkClient: ZkClient, topic: String,
                             ...
                             controllerEpoch: Int, acl: java.util.List[ACL]) {
      ...
      leaderForPartition => {
        ...
        try{
          ...
          ZkUtils.updatePersistentPath(zkClient, ZkUtils.getTopicPartitionLeaderAndIsrPath(topic, partition),
            ZkUtils.leaderAndIsrZkData(newLeaderAndIsr, controllerEpoch),acl=acl)
        } catch {
