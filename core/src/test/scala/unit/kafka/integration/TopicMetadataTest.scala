...
class TopicMetadataTest extends JUnit3Suite with ZooKeeperTestHarness {
  ...
  def testBasicTopicMetadata {
    // create topic
    val topic = "test"
    createTopic(zkClient, topic, numPartitions = 1, replicationFactor = 1, servers = Seq(server1), acl=null)
  ...
  def testGetAllTopicMetadata {
    ...
    createTopic(zkClient, topic1, numPartitions = 1, replicationFactor = 1, servers = Seq(server1), acl=null)
    createTopic(zkClient, topic2, numPartitions = 1, replicationFactor = 1, servers = Seq(server1), acl=null)
