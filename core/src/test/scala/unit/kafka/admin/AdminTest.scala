...
class AdminTest extends JUnit3Suite with ZooKeeperTestHarness with Logging {
  ...
  def testManualReplicaAssignment() {
    ...
    TestUtils.createBrokersInZk(zkClient, brokers, null)

    // duplicate brokers
    intercept[IllegalArgumentException] {
      AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, "test", Map(0->Seq(0,0)), acl=null)
    }

    // inconsistent replication factor
    intercept[IllegalArgumentException] {
      AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, "test", Map(0->Seq(0,1), 1->Seq(0)), acl=null)
    }

    // good assignment
    val assignment = Map(0 -> List(0, 1, 2),
                         1 -> List(1, 2, 3))
    AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, "test", assignment, acl=null)
  ...
  def testTopicCreationInZK() {
    ...
    val topic = "test"
    TestUtils.createBrokersInZk(zkClient, List(0, 1, 2, 3, 4),null)
    // create the topic
    AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, topic, expectedReplicaAssignment, acl=null)
    // create leaders for all partitions
    TestUtils.makeLeaderForPartition(zkClient, topic, leaderForPartitionMap, 1, acl=null)
  ...
  def testPreferredReplicaJsonData() {
    ...
    PreferredReplicaLeaderElectionCommand.writePreferredReplicaElectionData(zkClient, partitionsForPreferredReplicaElection, acl=null)
  ...
  def testBasicPreferredReplicaElection() {
    ...
    val preferredReplicaElection = new PreferredReplicaLeaderElectionCommand(zkClient, Set(TopicAndPartition(topic, partition)), acl=null)
  ...
  def testShutdownBroker() {
    ...
    val servers = serverConfigs.reverse.map(s => TestUtils.createServer(s))
    // create the topic
    TestUtils.createTopic(zkClient, topic, partitionReplicaAssignment = expectedReplicaAssignment, servers = servers, acl=null)
  ...
  def testTopicConfigChange() {
    ...
    try {
      ...
      AdminUtils.createTopic(server.zkClient, topic, partitions, 1, makeConfig(maxMessageSize, retentionMs), acl=null)
      checkConfig(maxMessageSize, retentionMs)

      // now double the config values for the topic and check that it is applied
      AdminUtils.changeTopicConfig(server.zkClient, topic, makeConfig(2*maxMessageSize, 2 * retentionMs), acl=null)
