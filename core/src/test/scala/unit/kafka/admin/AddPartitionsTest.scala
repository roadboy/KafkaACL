...
class AddPartitionsTest extends JUnit3Suite with ZooKeeperTestHarness {
  ...
  override def setUp() {
    ...
    // create topics first
    createTopic(zkClient, topic1, partitionReplicaAssignment = Map(0->Seq(0,1)), servers = servers, acl=null)
    createTopic(zkClient, topic2, partitionReplicaAssignment = Map(0->Seq(1,2)), servers = servers, acl=null)
    createTopic(zkClient, topic3, partitionReplicaAssignment = Map(0->Seq(2,3,0,1)), servers = servers, acl=null)
    createTopic(zkClient, topic4, partitionReplicaAssignment = Map(0->Seq(0,3)), servers = servers, acl=null)
  }
  ...
  def testTopicDoesNotExist {
    try {
      AdminUtils.addPartitions(zkClient, "Blah", 1, acl=null)
  ...
  def testWrongReplicaCount {
    try {
      AdminUtils.addPartitions(zkClient, topic1, 2, "0:1,0:1:2", acl=null)
  ...
  def testIncrementPartitions {
    AdminUtils.addPartitions(zkClient, topic1, 3, acl=null)
  ...
  def testManualAssignmentOfReplicas {
    AdminUtils.addPartitions(zkClient, topic2, 3, "1:2,0:1,2:3", acl=null)
    // wait until leader is elected
  ...
  def testReplicaPlacement {
    AdminUtils.addPartitions(zkClient, topic3, 7, acl=null)
