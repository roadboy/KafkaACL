...
class KafkaServer(val config: KafkaConfig, time: Time = SystemTime) extends Logging with KafkaMetricsGroup {
  ...
  private def initZk(): ZkClient = {
    ...
    if (chroot.length > 1) {
      ...
      ZkUtils.makeSurePersistentPathExists(zkClientForChrootCreation, chroot, null)
      info("Created zookeeper path " + chroot)
      zkClientForChrootCreation.close()
    }

    val zkClient = new ZkClient(config.zkConnect, config.zkSessionTimeoutMs, config.zkConnectionTimeoutMs, ZKStringSerializer)
    ZkUtils.setupCommonPaths(zkClient, null)
  ...
  private def generateBrokerId: Int = {
    try {
      ZkUtils.getBrokerSequenceId(zkClient, config.maxReservedBrokerId, null)
    } catch {
      case e: Exception =>
