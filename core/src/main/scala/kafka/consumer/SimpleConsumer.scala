...
class SimpleConsumer(val host: String,
   ...
   */
  def earliestOrLatestOffset(topicAndPartition: TopicAndPartition, earliestOrLatest: Long, consumerId: Int, aclAuth: String, zkConnect: String): Long = {
    val request = OffsetRequest(requestInfo = Map(topicAndPartition -> PartitionOffsetRequestInfo(earliestOrLatest, 1)),
                                clientId = clientId,
                                replicaId = consumerId,aclAuth = aclAuth, zkConnect = zkConnect)
