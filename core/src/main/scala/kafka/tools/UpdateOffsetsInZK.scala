...
object UpdateOffsetsInZK {
  ...
  def main(args: Array[String]) {
    ...
    zkClient.addAuthInfo("digest", config.aclAuth.getBytes)
    ...
    for (partition <- partitions) {
      ...
      ZkUtils.getBrokerInfo(zkClient, broker) match {
        case Some(brokerInfo) =>
          ...
          val request = OffsetRequest(Map(topicAndPartition -> PartitionOffsetRequestInfo(offsetOption, 1)),aclAuth = config.aclAuth, zkConnect = config.zkConnect)
          ...
          ZkUtils.updatePersistentPath(zkClient, topicDirs.consumerOffsetDir + "/" + partition, offset.toString, zkClient.getAcl(ZkUtils.getTopicPath(topic), new Stat()))
          numParts += 1
