...
object GetOffsetShell {
  ...
  def main(args: Array[String]): Unit = {
   ...
   if(args.length == 0)
    ...
    val topicsMetadata = ClientUtils.fetchTopicMetadata(Set(topic), metadataTargetBrokers, clientId, maxWaitMs, 0).topicsMetadata
        ...
        case Some(metadata) =>
          metadata.leader match {
            case Some(leader) =>
              ...
              val request = OffsetRequest(Map(topicAndPartition -> PartitionOffsetRequestInfo(time, nOffsets)),aclAuth = aclAuth, zkConnect = zkConnect)
