...
object TestLogCleaning {
  ...
  def main(args: Array[String]) {
    ...
    val dumpOpt = parser.accepts("dump", "Dump the message contents of a topic partition that contains test data from this test to standard out.")
                        ...
                        .ofType(classOf[String])
    val aclAuthOpt = parser.accepts("aclAuth", "ACL Auth")
                             .withRequiredArg
                             .describedAs("auth")
                             .ofType(classOf[String])
    ...
    val consumedDataFile = consumeMessages(zkUrl, topics, aclAuth)
  ...
  def makeConsumer(zkUrl: String, topics: Array[String], aclAuth: String): ZookeeperConsumerConnector = {
    ...
    consumerProps.setProperty("zookeeper.aclAuth", aclAuth)
  ...
  def consumeMessages(zkUrl: String, topics: Array[String], aclAuth: String): File = {
    val connector = makeConsumer(zkUrl, topics, aclAuth)
