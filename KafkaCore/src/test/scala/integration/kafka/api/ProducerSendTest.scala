...
class ProducerSendTest extends JUnit3Suite with KafkaServerTestHarness {
  ...
  def testSendOffset() {
    ...
    try {
      // create topic
      TestUtils.createTopic(zkClient, topic, 1, 2, servers, acl=null)
  ...
  def testSendToPartition() {
    ...
    try {
      // create topic
      val leaders = TestUtils.createTopic(zkClient, topic, 2, 2, servers, acl=null)
      val partition = 1
  ...
  def testFlush() {
    ...
    try {
      TestUtils.createTopic(zkClient, topic, 2, 2, servers, acl=null)
