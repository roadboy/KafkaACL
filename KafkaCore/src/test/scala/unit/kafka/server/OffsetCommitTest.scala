...
class OffsetCommitTest extends JUnit3Suite with ZooKeeperTestHarness {
  ...
  def testUpdateOffsets() {
    ...
    // create the topic
    createTopic(zkClient, topic, partitionReplicaAssignment = expectedReplicaAssignment, servers = Seq(server), acl=null)
  ...
  def testCommitAndFetchOffsets() {
    ...
    createTopic(zkClient, topic1, servers = Seq(server), numPartitions = 1, acl=null)
    createTopic(zkClient, topic2, servers = Seq(server), numPartitions = 2, acl=null)
    createTopic(zkClient, topic3, servers = Seq(server), numPartitions = 1, acl=null)
    createTopic(zkClient, topic4, servers = Seq(server), numPartitions = 1, acl=null)
  ...
  def testNonExistingTopicOffsetCommit() {
    ...
    createTopic(zkClient, topic2, servers = Seq(server), numPartitions = 1, acl=null)
