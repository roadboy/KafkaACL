...
class DeleteConsumerGroupTest extends JUnit3Suite with KafkaServerTestHarness {
  ...
  def testGroupWideDeleteInZK() {
    ...
    TestUtils.createTopic(zkClient, topic, 1, 3, servers, acl=null)
  ...
  def testGroupTopicWideDeleteInZKForGroupConsumingMultipleTopics() {
    ...
    val groupToDelete = "groupToDelete"
    val otherGroup = "otherGroup"
    TestUtils.createTopic(zkClient, topicToDelete, 1, 3, servers, acl=null)
    TestUtils.createTopic(zkClient, otherTopic, 1, 3, servers, acl=null)
  ...
  private def fillInConsumerGroupInfo(topic: String, group: String, consumerId: String, partition: Int, offset: Int, registerConsumer: Boolean) {
    ...
    val consumerConfig = new ConsumerConfig(consumerProps)
    val dir = new ZKGroupTopicDirs(group, topic)
    TestUtils.updateConsumerOffset(consumerConfig, dir.consumerOffsetDir + "/" + partition, offset, acl=null)
    ZkUtils.createEphemeralPathExpectConflict(zkClient, ZkUtils.getConsumerPartitionOwnerPath(group, topic, partition), "", acl=null)
    ZkUtils.makeSurePersistentPathExists(zkClient, dir.consumerRegistryDir, acl=null)
    if (registerConsumer) {
      ZkUtils.createEphemeralPathExpectConflict(zkClient, dir.consumerRegistryDir + "/" + consumerId, "", acl=null)
    }
  }
