...
class SyncProducerTest extends JUnit3Suite with KafkaServerTestHarness {
  ...
  def testMessageSizeTooLarge() {
    ...
    val producer = new SyncProducer(new SyncProducerConfig(props))
    TestUtils.createTopic(zkClient, "test", numPartitions = 1, replicationFactor = 1, servers = servers, acl=null)
  ...
  def testMessageSizeTooLargeWithAckZero() {
    ...
    val producer = new SyncProducer(new SyncProducerConfig(props))
    AdminUtils.createTopic(zkClient, "test", 1, 1, acl=null)
  ...
  def testProduceCorrectlyReceivesResponse() {
    ...
    AdminUtils.createTopic(zkClient, "topic1", 1, 1, acl=null)
    TestUtils.waitUntilLeaderIsElectedOrChanged(zkClient, "topic1", 0)
    AdminUtils.createTopic(zkClient, "topic3", 1, 1, acl=null)
  ...
  def testNotEnoughReplicas()  {
    ...
    AdminUtils.createTopic(zkClient, topicName, 1, 1,topicProps, acl=null)
