...
class UncleanLeaderElectionTest extends JUnit3Suite with ZooKeeperTestHarness {
  ...
  def testUncleanLeaderElectionEnabled {
    ...
    AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, topic, Map(partitionId -> Seq(brokerId1, brokerId2)), acl=null)
  ...
  def testUncleanLeaderElectionInvalidTopicOverride {
    ...
    intercept[ConfigException] {
      AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, topic, Map(partitionId -> Seq(brokerId1)), topicProps, acl=null)
    }
  }
