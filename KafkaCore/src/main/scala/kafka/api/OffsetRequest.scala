...
object OffsetRequest {
  ...
  def readFrom(buffer: ByteBuffer): OffsetRequest = {
    ...
    val aclAuth = readShortString(buffer)
    val zkConnect = readShortString(buffer)
    OffsetRequest(Map(pairs:_*), versionId= versionId, clientId = clientId, correlationId = correlationId, replicaId = replicaId, aclAuth= aclAuth, zkConnect= zkConnect)
  }
}
...
case class OffsetRequest(requestInfo: Map[TopicAndPartition, PartitionOffsetRequestInfo],
  ...
  def this(requestInfo: Map[TopicAndPartition, PartitionOffsetRequestInfo], correlationId: Int, replicaId: Int, aclAuth: String, zkConnect: String) = this(requestInfo, OffsetRequest.CurrentVersion, correlationId, OffsetRequest.DefaultClientId, replicaId, aclAuth, zkConnect)
  ...
  def writeTo(buffer: ByteBuffer) {
    ...
    writeShortString(buffer, aclAuth)
    writeShortString(buffer, zkConnect)
  }
  ...
  def sizeInBytes =
    ...
    shortStringLength(aclAuth) +
    shortStringLength(zkConnect) 
    )
  ...
  override def describe(details: Boolean): String = {
    ...
    offsetRequest.append("; aclAuth: " + aclAuth)
    offsetRequest.append("; zkConnect: " + zkConnect)
