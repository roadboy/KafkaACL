...
object TopicCommand {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      if(opts.options.has(opts.createOpt)){
        ...
        if(opts.options.has(opts.aclCreator)){         
          val id = new Id("digest",DigestAuthenticationProvider.generateDigest(opts.options.valueOf(opts.aclCreator)))   
        ...
        createTopic(zkClient, opts, acl)
      }
        
      else if(opts.options.has(opts.alterOpt)){
        val acl = new ArrayList[ACL]()
        val aclTmp = new ArrayList[ACL]()
        
        val topic = opts.options.valueOf(opts.topicOpt)
        val pathConfigTopic = ZkUtils.getTopicConfigPath(topic)
        val pathTopic = ZkUtils.getTopicPath(topic)
        
        val aclOri = zkClient.getAcl(pathTopic, new Stat())
        
        if(opts.options.has(opts.aclCreator)){
          
          
          if(opts.options.has(opts.toPrivateOpt)){
            val id = new Id("digest",DigestAuthenticationProvider.generateDigest(opts.options.valueOf(opts.aclCreator)))   
            acl.add(new ACL(ZooDefs.Perms.ALL, id))
            ...
            zkClient.setAcl(pathConfigTopic, acl);
            zkClient.setAcl(pathTopic, acl);
            
            zkClient.addAuthInfo("digest", opts.options.valueOf(opts.aclCreator).getBytes)
          }else{
            
            if(zkClient.exists(pathTopic)){
              val aclCreatorList = new ArrayList[ACL]()
              val aclOri = zkClient.getAcl(pathTopic, new Stat())
              if(aclOri!=null){
                for (a:ACL <- aclOri.asScala ){
                  if(a.getId().getId().equals(DigestAuthenticationProvider.generateDigest(opts.options.valueOf(opts.aclCreator)))){
                        aclCreatorList.add(a)
                  }                                        
                }
                    
                if(aclCreatorList.size()!=1){
                  println("aclCreator is not exist,please check!")
                  System.exit(1)
                }
                if(aclCreatorList.get(0).getPerms!=31){
                  println("aclCreator is not owner or admin,please check!")
                  System.exit(1)
                }
              } 
            }
          ...
          if(opts.options.has(opts.addAcl)){                   
            val id = new Id("digest",DigestAuthenticationProvider.generateDigest(opts.options.valueOf(opts.addAcl)))   
  
            if(zkClient.exists(pathTopic)){
              if(aclOri!=null){
                  for (a:ACL <- aclOri.asScala ){
                    if(a.getId().getId().equals(id.getId)){
                      println("the user:passwd already exist,please input another!")
                      System.exit(1)
                    }else
                      acl.add(a)
                  }
              } 
              val aNew = new ACL(ZooDefs.Perms.READ,id)
              acl.add(aNew)
          ...
          if(opts.options.has(opts.updateAcl)){  
            
            val oldUser = opts.options.valueOf(opts.updateAcl).split(",")(0)
            val newUser = opts.options.valueOf(opts.updateAcl).split(",")(1)            
            val id = new Id("digest",DigestAuthenticationProvider.generateDigest(newUser))   

            if(zkClient.exists(pathTopic)){
              if(aclOri!=null){                                    
                  for (a:ACL <- aclOri.asScala ){
                    if(!(a.getId().getId().equals(DigestAuthenticationProvider.generateDigest(oldUser)))&& (! a.getId().getId().equals(DigestAuthenticationProvider.generateDigest(newUser))))
                      acl.add(a)
                    else
                      aclTmp.add(a)
                  }
              ...
              if(aclTmp.size()!=1){
                println("user:oldpasswd is wrong!")
                System.exit(1)
              }
             
              val aNew = new ACL(aclTmp.get(0).getPerms,id)
              acl.add(aNew)
          ...
          if(opts.options.has(opts.deleteAcl)){  
            
            val deleteUser = opts.options.valueOf(opts.deleteAcl)            
            val id = new Id("digest",DigestAuthenticationProvider.generateDigest(deleteUser))   
            
            if(zkClient.exists(pathTopic)){
              if(aclOri!=null){
                  for (a:ACL <- aclOri.asScala ){
                    if(!a.getId().getId().equals(DigestAuthenticationProvider.generateDigest(deleteUser)))
                      acl.add(a)
                    else
                      aclTmp.add(a)                                        
                  }
                  ...
                  if(aclTmp.size()!=1){
                    println("deleteAcl is not exist,please check!")
                    System.exit(1)
                  }
                  if(aclTmp.get(0).getPerms==31){
                    println("deleteAcl is a owner or admin,can not delete,please check!")
                    System.exit(1)
                  }
              } 
          ...
          if(opts.options.has(opts.toPublicOpt)){                                        
            zkClient.setAcl(pathConfigTopic, Ids.OPEN_ACL_UNSAFE);
            zkClient.setAcl(pathTopic, Ids.OPEN_ACL_UNSAFE);
          }
          
          
        }
        alterTopic(zkClient, opts, acl)
      }
        
      else if(opts.options.has(opts.listOpt))
        listTopics(zkClient, opts)
      else if(opts.options.has(opts.describeOpt)){
        if(opts.options.has(opts.setAcl)){
          zkClient.addAuthInfo("digest", opts.options.valueOf(opts.setAcl).getBytes)
        }
  ...
  def createTopic(zkClient: ZkClient, opts: TopicCommandOptions, acl: java.util.List[ACL]) {
    val topic = opts.options.valueOf(opts.topicOpt)
    val configs = parseTopicConfigsToBeAdded(opts)
    if (opts.options.has(opts.replicaAssignmentOpt)) {
      val assignment = parseReplicaAssignment(opts.options.valueOf(opts.replicaAssignmentOpt))
      AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, topic, assignment, configs, false, acl)
    } else {
      CommandLineUtils.checkRequiredArgs(opts.parser, opts.options, opts.partitionsOpt, opts.replicationFactorOpt)
      val partitions = opts.options.valueOf(opts.partitionsOpt).intValue
      val replicas = opts.options.valueOf(opts.replicationFactorOpt).intValue
      AdminUtils.createTopic(zkClient, topic, partitions, replicas, configs, acl)
    }
    println("Created topic \"%s\".".format(topic))
  }

  def alterTopic(zkClient: ZkClient, opts: TopicCommandOptions, acl: java.util.List[ACL]) {
    ...
    topics.foreach { topic =>
      ...
      if(opts.options.has(opts.configOpt) || opts.options.has(opts.deleteConfigOpt)) {
        ...
        AdminUtils.changeTopicConfig(zkClient, topic, configs, acl)
      ...
      if(opts.options.has(opts.partitionsOpt)) {
        ...
        AdminUtils.addPartitions(zkClient, topic, nPartitions, replicaAssignmentStr, config = configs, acl=acl)
  ...
  def deleteTopic(zkClient: ZkClient, opts: TopicCommandOptions, acl: java.util.List[ACL]) {
    val topics = getTopics(zkClient, opts)
    if (topics.length == 0) {
      println("Topic %s does not exist".format(opts.options.valueOf(opts.topicOpt)))
    }
    topics.foreach { topic =>
      try {
        ZkUtils.createPersistentPath(zkClient, ZkUtils.getDeleteTopicPath(topic), acl=acl)
  ...
  class TopicCommandOptions(args: Array[String]) {
    ...
    val aclCreator = parser.accepts("aclCreator", "addACL for Topic Creator,Use with Create"+nl+"Usage:"+nl+"--aclCreator user:passwd")
                             .withRequiredArg
                             .describedAs("aclCreator")
                             .ofType(classOf[String])
                             
    val setAcl = parser.accepts("setAcl", "setACL for  describe Topic if this topic has ACL,Use with Describe")
                             .withRequiredArg
                             .describedAs("setAcl")
                             .ofType(classOf[String])
    val addAcl = parser.accepts("addAcl", "Creator add new ACL for other user(only read) for Topic,Use with Alter"+nl+"Usage:"+nl+"--aclCreator:owner:ownerpw"+nl+"--addAcl user:passwd")
                             .withRequiredArg
                             .describedAs("addAcl")
                             .ofType(classOf[String])
                             
    val updateAcl = parser.accepts("updateAcl", "Creator update exist ACL for Topic,Use with Alter"+nl+"Usage:"+nl+"--aclCreator:owner:ownerpw"+nl+"--updateAcl user:oldPasswd,user:newPasswd ")
                             .withRequiredArg
                             .describedAs("updateAcl")
                             .ofType(classOf[String])
    
    val deleteAcl = parser.accepts("deleteAcl", "Creator delete exist ACL for Topic,Use with Alter"+nl+"Usage:"+nl+"--aclCreator:owner:ownerpw"+nl+"--deleteAcl user:passwd ")
                             .withRequiredArg
                             .describedAs("deleteAcl")
                             .ofType(classOf[String])
                             
    val toPublicOpt = parser.accepts("toPublic", "Change the topic from private to public")
    val toPrivateOpt = parser.accepts("toPrivate", "Change the topic from public to private")
    ...
    def checkArgs() {
      ...
      if (options.has(addAcl))
        CommandLineUtils.checkRequiredArgs(parser, options, aclCreator)
      if (options.has(updateAcl))
        CommandLineUtils.checkRequiredArgs(parser, options, aclCreator)
      if (options.has(deleteAcl))
        CommandLineUtils.checkRequiredArgs(parser, options, aclCreator)
      if (options.has(toPublicOpt))
        CommandLineUtils.checkRequiredArgs(parser, options, aclCreator)
      if (options.has(toPrivateOpt))
        CommandLineUtils.checkRequiredArgs(parser, options, aclCreator)
      ...
      CommandLineUtils.checkInvalidArgs(parser, options, aclCreator, allTopicLevelOpts -- Set(alterOpt, createOpt)) 
      CommandLineUtils.checkInvalidArgs(parser, options, addAcl, allTopicLevelOpts -- Set(alterOpt)) 
      CommandLineUtils.checkInvalidArgs(parser, options, updateAcl, allTopicLevelOpts -- Set(alterOpt))
      CommandLineUtils.checkInvalidArgs(parser, options, deleteAcl, allTopicLevelOpts -- Set(alterOpt))
      CommandLineUtils.checkInvalidArgs(parser, options, setAcl, allTopicLevelOpts -- Set(describeOpt))
      CommandLineUtils.checkInvalidArgs(parser, options, toPublicOpt, allTopicLevelOpts -- Set(alterOpt))
      CommandLineUtils.checkInvalidArgs(parser, options, toPrivateOpt, allTopicLevelOpts -- Set(alterOpt))
    }
  }
