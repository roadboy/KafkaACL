...
object AdminUtils extends Logging {
  ...
  */
  def addPartitions(zkClient: ZkClient,
    ...
    AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, topic, partitionReplicaList, config, true, acl)
  }
  ...
  def deleteTopic(zkClient: ZkClient, topic: String, acl: java.util.List[ACL]) {
    ZkUtils.createPersistentPath(zkClient, ZkUtils.getDeleteTopicPath(topic),"", acl)
  }
  ...
  def createTopic(zkClient: ZkClient,
    ...
    AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(zkClient, topic, replicaAssignment, topicConfig, false, acl=acl)
  }
  ...
  def createOrUpdateTopicPartitionAssignmentPathInZK(zkClient: ZkClient,
    ...
    writeTopicConfig(zkClient, topic, config, acl)
    
    // create the partition assignment
    writeTopicPartitionAssignment(zkClient, topic, partitionReplicaAssignment, update, acl)
  }
  
  private def writeTopicPartitionAssignment(zkClient: ZkClient, topic: String, replicaAssignment: Map[Int, Seq[Int]], update: Boolean, acl: java.util.List[ACL]) {
    try {
      ...
      if (!update) {
        ...
        ZkUtils.createPersistentPath(zkClient, zkPath, jsonPartitionData, acl)
      } else {
        info("Topic update " + jsonPartitionData.toString)
        ZkUtils.updatePersistentPath(zkClient, zkPath, jsonPartitionData, acl)
      }
   ...
   *
   */
  def changeTopicConfig(zkClient: ZkClient, topic: String, configs: Properties, acl: java.util.List[ACL]) {
    ...
    writeTopicConfig(zkClient, topic, configs, acl)
   ...
   */
  private def writeTopicConfig(zkClient: ZkClient, topic: String, config: Properties, acl: java.util.List[ACL]) {
    val configMap: mutable.Map[String, String] = {
      import JavaConversions._
      config
    }
    val map = Map("version" -> 1, "config" -> configMap)
    ZkUtils.updatePersistentPath(zkClient, ZkUtils.getTopicConfigPath(topic), Json.encode(map), acl)
  }
