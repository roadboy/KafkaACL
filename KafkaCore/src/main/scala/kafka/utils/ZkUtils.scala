...
object ZkUtils extends Logging {
  ...
  def setupCommonPaths(zkClient: ZkClient, acl: java.util.List[ACL]) {
    for(path <- Seq(ConsumersPath, BrokerIdsPath, BrokerTopicsPath, TopicConfigChangesPath, TopicConfigPath,
      ...
      makeSurePersistentPathExists(zkClient, path, acl)
  }
  ...
  /** returns a sequence id generated by updating BrokerSequenceIdPath in Zk.
    ...
    */
  def getBrokerSequenceId(zkClient: ZkClient, MaxReservedBrokerId: Int, acl: java.util.List[ACL]): Int = {
    getSequenceId(zkClient, BrokerSequenceIdPath, acl) + MaxReservedBrokerId
  }
  ...
  def registerBrokerInZk(zkClient: ZkClient, id: Int, host: String, port: Int, timeout: Int, jmxPort: Int, acl: java.util.List[ACL]) {
   ...
   */
  def makeSurePersistentPathExists(client: ZkClient, path: String, acl: java.util.List[ACL]) {
    if (!client.exists(path)&&acl==null)
      new ZkPath(client).createPersistent(path, true) // won't throw NoNodeException or NodeExistsException
    if (!client.exists(path)&&acl!=null)
      new ZkPath(client).createPersistent(path, true, acl)
  }

  /**
   *  create the parent path
   */
  private def createParentPath(client: ZkClient, path: String, acl: java.util.List[ACL]): Unit = {
    val parentDir = path.substring(0, path.lastIndexOf('/'))
    if (parentDir.length != 0) {
      if(acl!=null&&acl.size()>0)
        new ZkPath(client).createPersistent(parentDir, true, acl)
      else
   ...
   */
  private def createEphemeralPath(client: ZkClient, path: String, data: String, acl: java.util.List[ACL]): Unit = {
    val zkPath = new ZkPath(client)
    try {
      zkPath.createEphemeral(path, data)
    } catch {
      case e: ZkNoNodeException => {
        createParentPath(client, path, acl)
        zkPath.createEphemeral(path, data)
      }
    }
  }
   ...
   */
  def createEphemeralPathExpectConflict(client: ZkClient, path: String, data: String, acl: java.util.List[ACL]): Unit = {
    try {
      createEphemeralPath(client, path, data, acl)
    } catch {
      case e: ZkNodeExistsException => {
   ...
   */
  def createEphemeralPathExpectConflictHandleZKBug(zkClient: ZkClient, path: String, data: String, expectedCallerData: Any, checker: (String, Any) => Boolean, backoffTime: Int, acl: java.util.List[ACL]): Unit = {
    while (true) {
      try {
        createEphemeralPathExpectConflict(zkClient, path, data, acl)
        return
      } catch {
        case e: ZkNodeExistsException => {
   ...
   */
  def createPersistentPath(client: ZkClient, path: String, data: String = "", acl: java.util.List[ACL]): Unit = {
    val zkPath = new ZkPath(client)
    try {
      if(acl!=null&&acl.size()>0)
   ...
   */
  def updatePersistentPath(client: ZkClient, path: String, data: String, acl: java.util.List[ACL]) = {
    try {
      ...
      case e: ZkNoNodeException => {
        ...
        try {
          if(acl!=null&&acl.size()>0)
            new ZkPath(client).createPersistent(path, data, acl)
          else
   ...
   * create parrent directory if necessary. Never throw NodeExistException.
   */
  def updateEphemeralPath(client: ZkClient, path: String, data: String, acl: java.util.List[ACL]): Unit = {
    try {
      client.writeData(path, data)
    } catch {
      case e: ZkNoNodeException => {
  ...
  def updatePartitionReassignmentData(zkClient: ZkClient, partitionsToBeReassigned: Map[TopicAndPartition, Seq[Int]], acl: java.util.List[ACL]) {
    ...
    partitionsToBeReassigned.size match {
      ...
      case _ =>
        ...
        try {
          updatePersistentPath(zkClient, zkPath, jsonData, acl)
          info("Updated partition reassignment path with %s".format(jsonData))
        } catch {
          case nne: ZkNoNodeException =>
            ZkUtils.createPersistentPath(zkClient, zkPath, jsonData, acl)
  ...
  /**
    ...
    */
  def getSequenceId(client: ZkClient, path: String, acl: java.util.List[ACL]): Int = {
    try {
      val stat = client.writeDataReturnStat(path, "", -1)
      return stat.getVersion
    } catch {
      case e: ZkNoNodeException => {
        createParentPath(client, BrokerSequenceIdPath, acl)
        try {
...
class ZkPath(client: ZkClient) {
  ...
  def createPersistent(path: String, data: Object, acl: java.util.List[ACL]) {
    //client.createPersistent(path, data)
    client.createPersistent(path, data, acl)
  }
  ...
  def createPersistent(path: String, createParents: Boolean, acl: java.util.List[ACL]) {
    client.createPersistent(path, createParents, acl)
  }
