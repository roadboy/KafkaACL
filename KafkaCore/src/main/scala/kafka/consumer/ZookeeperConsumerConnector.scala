...
private[kafka] class ZookeeperConsumerConnector(val config: ConsumerConfig,
  ...
  private def connectZk() {
    ...
    if(config.aclSchema!=null&&config.aclAuth!=null&&config.aclSchema.equals("digest"))
      zkClient.addAuthInfo(config.aclSchema,config.aclAuth.getBytes)
  }
  ...
  private def registerConsumerInZK(dirs: ZKGroupDirs, consumerIdString: String, topicCount: TopicCount) {
    ...
    createEphemeralPathExpectConflictHandleZKBug(zkClient, dirs.consumerRegistryDir + "/" + consumerIdString, consumerRegistrationInfo, null,
                                                 (consumerZKString, consumer) => true, config.zkSessionTimeoutMs, null)
  ...
  def commitOffsetToZooKeeper(topicPartition: TopicAndPartition, offset: Long) {
    if (checkpointedZkOffsets.get(topicPartition) != offset) {
      ...
      updatePersistentPath(zkClient, topicDirs.consumerOffsetDir + "/" + topicPartition.partition, offset.toString, null)
  ...
  class ZKSessionExpireListener(val dirs: ZKGroupDirs,
    ...
    def handleSessionEstablishmentError(error: Throwable) { }
  ...
  class ZKRebalancerListener(val group: String, val consumerIdString: String,
    ...
    private def reflectPartitionOwnershipDecision(partitionAssignment: Map[TopicAndPartition, ConsumerThreadId]): Boolean = {
      ...
      val partitionOwnershipSuccessful = partitionAssignment.map { partitionOwner =>
        ...
        try {
          createEphemeralPathExpectConflict(zkClient, partitionOwnerPath, consumerThreadId.toString, null)
