...
object ConsumerOffsetChecker extends Logging {
  ...
  private def processPartition(zkClient: ZkClient,
                               group: String, topic: String, pid: Int, aclAuth: String, zkConnect: String) {
      ...
      case Some(bid) =>
        ...
        consumerOpt match {
          case Some(consumer) =>
            ...
            val request =
              OffsetRequest(immutable.Map(topicAndPartition -> PartitionOffsetRequestInfo(OffsetRequest.LatestTime, 1)),aclAuth = aclAuth, zkConnect = zkConnect)
  ...
  private def processTopic(zkClient: ZkClient, group: String, topic: String, aclAuth: String, zkConnect: String) {
    topicPidMap.get(topic) match {
      case Some(pids) =>
        pids.sorted.foreach {
          pid => processPartition(zkClient, group, topic, pid, aclAuth, zkConnect)
        }
      case None => // ignore
    }
  }
  ...
  def main(args: Array[String]) {
    ...
    val aclAuthOpt = parser.accepts("setAcl", "Topics ACL").
    ...
    val aclAuth = options.valueOf(aclAuthOpt)
    ...
    try {
      ...
      if(aclAuth!=null&&aclAuth.ne(""))
        zkClient.addAuthInfo("digest", aclAuth.getBytes)
      ...
      topicList.sorted.foreach {
        topic => processTopic(zkClient, group, topic, aclAuth, zkConnect)
      }
      ...
      case t: Throwable =>
        println("Exiting due to: %s.".format(t.printStackTrace()))
    }
    finally {
