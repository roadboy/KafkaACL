...
object ReplicaVerificationTool extends Logging {
  ...
  def main(args: Array[String]): Unit = {
   ...
   if(args.length == 0)
    ...
    val topicsMetadataResponse = ClientUtils.fetchTopicMetadata(Set[String](), metadataTargetBrokers, clientId, maxWaitMs, 0)
    ...
    val replicaBuffer = new ReplicaBuffer(expectedReplicasPerTopicAndPartition,
                                          ...
                                          brokerMap,
                                          initialOffsetTime,
                                          reportInterval,aclAuth, zkConnect)
...
private class ReplicaBuffer(expectedReplicasPerTopicAndPartition: Map[TopicAndPartition, Int],
                            ...
                            reportInterval: Long,
                            aclAuth: String, zkConnect: String) extends Logging {
  ...
  private def initialize(aclAuth: String, zkConnect: String) {
    ...
    setInitialOffsets(aclAuth, zkConnect)
  }
  ...
  private def setInitialOffsets(aclAuth: String, zkConnect: String) {
    for ((brokerId, topicAndPartitions) <- leadersPerBroker) {
      ...
      val offsetRequest = OffsetRequest(initialOffsetMap,aclAuth = aclAuth, zkConnect = zkConnect)
