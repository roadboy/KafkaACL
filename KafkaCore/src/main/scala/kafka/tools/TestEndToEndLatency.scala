...
object TestEndToEndLatency {
  def main(args: Array[String]) {
    if (args.length != 8) {
      System.err.println("USAGE: java " + getClass().getName + " broker_list zookeeper_connect topic num_messages consumer_fetch_max_wait producer_acks aclauth")
      System.exit(1)
    }
    ...
    consumerProps.put("zookeeper.aclAuth", aclAuth)
