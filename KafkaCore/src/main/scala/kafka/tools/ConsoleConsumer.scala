...
object ConsoleConsumer extends Logging {
  ...
  def main(args: Array[String]) {
    ...
    val zkConnectOpt = parser.accepts("zookeeper", "REQUIRED: The connection string for the zookeeper connection in the form host:port. " +
            ...
            .describedAs("urls")
            .ofType(classOf[String])
    val setAcl = parser.accepts("setAcl", "setACL for  Consume Topic if this topic has ACL,Use with Consume")
                             .withRequiredArg
                             .describedAs("setAcl")
                             .ofType(classOf[String])
    ...
    if(options.valueOf(setAcl)!=null)
      consumerProps.put("zookeeper.aclAuth", options.valueOf(setAcl))
